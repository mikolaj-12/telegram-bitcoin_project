...
BOT_TOKEN = '8244344911:AAEZMaGVNgfhEtKA6b9CAvkcAkPCKAFgiY8'
...
def predict_price(t, n):
    ...
    match t:
        case '1h':
            net = load_model('models/1h_model.keras')
            dim = 4
        case '4h':
            net = load_model('models/4h_model.keras')
            dim = 5
        case '1d':
            net = load_model('models/1d_model.keras')
            dim = 5
    ...
    for i in range(n):
        ...
        # Generowanie kolejnej daty – dodajemy 1 jednostkę czasu
        match t:
            case '1h':
...
async def handle_periods(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # await context.bot.send_photo(chat_id=query.message.chat_id, photo=chart_buffer,
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(handle_timeframe, pattern=r'^timeframe_'))
    app.add_handler(CallbackQueryHandler(handle_periods, pattern=r'^periods_'))
    print("Bot starting...")
    app.run_polling()
